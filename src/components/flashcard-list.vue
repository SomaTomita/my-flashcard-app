<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <h2 class="mt-6 mb-10 text-center text-3xl font-extrabold text-gray-900">FlashCard List</h2>
    <!-- カードが存在しない場合のメッセージ -->
    ...
    <!-- カード一覧 -->
    <div v-else class="p-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      <div
        v-for="card in cards"
        :key="card.id"
        class="m-2 bg-white p-6 rounded-lg shadow-md flex flex-col justify-between h-52 relative"
      ...
      >
        ...
        <div class="absolute bottom-2 right-2 flex space-x-2">
          <button
            ...
            class="text-xs bg-gray-200 hover:bg-cyan-300 text-cyan-800 px-3 py-1 rounded whitespace-nowrap"
          >
            Edit
          </button>
          <button
            @click.stop="confirmDelete(card)"
            class="text-xs bg-gray-200 hover:bg-red-300 text-cyan-800 px-3 py-1 rounded whitespace-nowrap"
          >
            Delete
          </button>
        </div>
